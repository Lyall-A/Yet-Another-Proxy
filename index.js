...
function handleRequest(http, connection, proxy) {
    ...
    function assignService(service) {
        ...
        if (publicAddress === (realAddress || address) && service.redirectPublicToLocal && service.localRedirectHost && serviceHost.endsWith(".")) return connection.bypass(307, "Temporary Redirect", [["Location", `${service.localRedirectProtocol ? formatString(service.localRedirectProtocol, formatStringObject) : config.ssl ? "https" : "http"}://${formatString(service.localRedirectHost, formatStringObject)}:${service.localRedirectPort ? formatString(service.localRedirectPort, formatStringObject) : config.port}${http.target}`]]);

        // Redirect to specific public address
        if (service.forcePublicHost && service.publicRedirectHost && hostname !== formatString(service.publicRedirectHost, formatStringObject) && serviceHost.endsWith(".")) return connection.bypass(307, "Temporary Redirect", [["Location", `${service.publicRedirectProtocol ? formatString(service.publicRedirectProtocol, formatStringObject) : config.ssl ? "https" : "http"}://${formatString(service.publicRedirectHost, formatStringObject)}:${service.publicRedirectPort ? formatString(service.publicRedirectPort, formatStringObject) : config.port}${http.target}`]]);
